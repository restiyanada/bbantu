...
broken this project twice — see ARCHITECTURE.md. Use `drizzle-kit migrate`, or
run the generated SQL by hand, then verify:
...
- **Journaled** (in `meta/_journal.json`) — generated by `drizzle-kit generate`
  and applied by `drizzle-kit migrate`.
- **Hand-written, not journaled** — `0004`, `0006`, `0009`. Pure SQL with no
  `schema.ts` change (pg_cron schedules, one-time data backfills). `db:push`
  and `migrate` never replay these; **run them by hand once** in the SQL editor.
  Each file's header says what it does and how to verify it.
...
3. Install and apply the schema:
   ```bash
   npm install
   npx drizzle-kit migrate     # not db:push — see the RLS warning above
   ```
   Then run the non-journaled migrations and the two `supabase/*_storage_setup.sql`
   files by hand in the SQL editor.
...
| `npx drizzle-kit migrate` | Apply journaled migrations (prefer over `db:push`) |
...
⚠️ You also need to add `public/_redirects` containing `/*  /index.html  200`
before the first deploy — it doesn't exist in the repo yet. Without it Pages
serves static files only, so any deep link (`/orders/:accessToken`,
`/dashboard`) 404s on refresh or when opened from an email. That breaks the
customer order tracker, which is reached by link and nothing else.
