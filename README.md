...
⚠️ **A policy that gates on `admin_users` needs `admin_users` to be readable.**
Nearly every staff-gated policy here is `exists (select 1 from admin_users
where email = auth.jwt() ->> 'email' and <permission> = true)`. That subquery
runs as the **caller**, so it is subject to `admin_users`' own RLS. `admin_users`
had RLS enabled and no policies, so `authenticated` saw zero rows and every one
of those gates evaluated to false — silently, because a policy returning false
is a denial, not an error. Migration `0011` adds the missing own-row SELECT
policy. If you ever add a table whose policy reads `admin_users`, it inherits
this behaviour for free now — but the same trap applies to any *other* RLS-on
table you reference from inside a policy.

It stayed hidden because the admin screens that go through Edge Functions
(orders, payments, pickup) run on the service-role connection, which bypasses
RLS. Only the screens writing straight from the browser — products, batches —
were affected.
...
- **Journaled** (in `meta/_journal.json`) — generated by `drizzle-kit generate`.
  ⚠️ Its filenames have drifted from the journal's own indices, so `generate`
  names each new migration and snapshot after the **next journal index**, not
  the next file number — `0011` was emitted as `0008_*.sql` with a
  `meta/0008_snapshot.json` that **overwrites the existing one**. After every
  `generate`: move the new snapshot aside, `git checkout` the one it clobbered,
  rename both files to the next free number, update the journal's last `tag`,
  and check the chain with
  `0011_snapshot.json`'s `prevId` == `0010_snapshot.json`'s `id`.
